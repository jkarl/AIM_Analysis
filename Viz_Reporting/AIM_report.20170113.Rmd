...
for (lev in ru.levels) { # by reporting unit level
  ...
  for (ru in rus) { # by specific reporting unit
    ...
    prop.table <-  prop.table[,-c(2,5,6,8)] %>% arrange(Management.Question,Evaluation.Category)  # sort by mgt question and category
    names(prop.table) <- c("Management Question","Indicator","Category","Required Percent","Estimated Percent","Lower CI","Upper CI","Objective Met?")
    cat("\n## Reporting Unit Level:",ru,"\n")
    print(xtable(prop.table,align=c("l","l","l","l","c","c","c","c","c")),type='html',include.rownames=FALSE, html.table.attributes=list("border='0' cellpadding='5' class='bordered_table' "))
  }
}
...
for (lev in ru.levels) { # by reporting unit level
  ...
  for (ru in rus) { # by specific reporting unit
    ...
    for (ind in Indicators) {  # by indicator
      ...
      req.prop <- as.numeric((benchmarks.sum[benchmarks.sum$indicator.tdat==ind & !is.na(benchmarks.sum$Required.Proportion),4]))
      cat("\n",req.prop,"\n")
      if (!is.na(req.prop)) {
