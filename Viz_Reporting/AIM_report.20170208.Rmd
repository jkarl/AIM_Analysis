...
indicatorMap.new <- function(level,ru,repunits.spdf,prjarea.spdf,samplepts.spdf) {
  runit <- repunits.spdf[repunits.spdf$Type==level&repunits.spdf$Sbppltn==ru,] # get the reporting unit
  ru.points <- over(samplepts.spdf,runit,returnList=F)
  m <- ggplot()+geom_polygon(data=fortify(prjarea.spdf),aes(x=long,y=lat),fill="white",color="black")+coord_map()
  if (level=="Study Area") {
    m<-m+geom_polygon(data=fortify(prjarea.spdf),aes(x=long,y=lat,group=group),fill="darkgray")
  } else {
  ...
  m<-m+geom_point(data=ru.points,aes(x=Longitude,y=Latitude,group=ProjectName),color="black")
  m<-m+theme(line=element_blank(),text=element_blank(),title=element_blank())
  return(m)
}
...
msg1 <- paste("This graph shows the estimated percentage of the reporting unit in different categories for the specified indicator. ",params$conf.level,"% confidence intervals (CI) around those estimates are provided as error bars. The yellow bar denotes the landscape criterion, or point at which an objective is deemed to be met or not met as defined in the Sample Design Worksheet.",sep="")
...
## Sample Design Information (point fate, stratification, study area bdy)
  ...
  point.fates <- design.pts.info %>% 
    melt(id="YEAR", measure.vars=c("Target Sampled","Inaccessible","Non-target","Unknown","Unneeded")) %>% 
...
```{r plotDates, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  g <- ggplot(data=as.data.frame(tdat.proj.spdf), aes(x=yday,fill=factor(year))) + geom_histogram(binwidth=(7)) + xlim(0,365) +
  ...
  for (ru in rus) {
    ...
    for (mq in mqs) { # by specific reporting unit
      ...
      for (ind in Indicators) {  # by indicator
        ...
        m <- indicatorMap.new(lev,ru,repUnits.spdf,prjarea.spdf,tdat.proj.spdf,ind,mq)
