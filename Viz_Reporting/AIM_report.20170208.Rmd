...
params:
  ...
  cat.analysis.infile: C:\Users\jasokarl\Google Drive\BLM_AIM\Analysis_Reporting\Bruneau\BFO_2015_test_analysis.csv
  project: Idaho Bruneau FO 2015 # Project Name from TerrADat
  project.shortname: Bruneau FO 2015
  conf.level: 80
  sdd.gdb: C:\Users\jasokarl\Documents\GitHub\AIM_Analysis\Viz_Reporting\Bruneau_Example\BFO_2015-2019_SDD_122116.gdb
  benchmarks.filename: C:\Users\jasokarl\Google Drive\BLM_AIM\Analysis_Reporting\Bruneau\DataExplorer_AnalysisSprint_BruneauFO.xlsx
  ...
  repunits.file: C:\Users\jasokarl\Google Drive\BLM_AIM\Analysis_Reporting\Bruneau\HUC_5_watersheds_in_BFO_geodd.shp
...
for (lev in ru.levels) { # by reporting unit level
  ...
  for (ru in rus) { # by specific reporting unit
    ...
    names(prop.table.ru) <- c("Management Question","Indicator","Category","Required Percent","Estimated Percent","Lower CI","Upper CI","Objective Met?")
    cat("\n## Reporting Unit Level:",ru,"\n")
    print(xtable(prop.table.ru,align=c("l","l","l","l","c","c","c","c","c")),type='html',include.rownames=FALSE, html.table.attributes=list("border='0' cellpadding='5' class='bordered_table' "))
  }
}
...
for (lev in ru.levels) { # by reporting unit level
  cat("\n## Reporting Unit Level: ",lev,"{.tabset}\n")
  #cat("\n",msg1,"\n")
  data.rulevel <- aim.analysis %>% filter(Type==lev)
  rus <- unique(data.rulevel$Subpopulation)
  for (ru in rus) { # by specific reporting unit
    ...
    for (ind in Indicators) {  # by indicator
      ...
      g <- indicatorPlot(aim.analysis,lev,ru,ind,req.prop)
