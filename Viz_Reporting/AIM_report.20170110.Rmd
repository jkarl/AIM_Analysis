---
title: "Draft BLM AIM Project Analysis Report"
...
params:
  ...
  cat.analysis.infile: C:\Users\jasokarl\Google Drive\Landscape Toolbox\LandscapeToolbox_GitHub\AIM_Analysis\analysis\analysis_output.csv
  project: New Mexico RGdNNM #"Rio Grande Del Norte NM"
  conf.level: 80
  sdd.gdb: C:\Users\jasokarl\Google Drive\BLM_AIM\geodata\TaFO_SDD_041215_dummy.gdb
...
## Objectives and benchmark information from the Excel template
  ...
  benchmarks.sum <- benchmarks %>% group_by(Management.Question,Classification) %>% summarise(
      indicator.name.alt=first(indicator.name.alt),
...
bm.table <- benchmarks.sum[,c(1,6,3,2,4)]
names(bm.table) <- c("Management Goal","Benchmark Source","Indicator","Classification","Required Proportion")
...
for (lev in ru.levels) { # by reporting unit level
  ...
  for (ru in rus) { # by specific reporting unit
    ...
    for (ind in Indicators) {  # by indicator
      ...
      grid.arrange(arrangeGrob(m,nrow=2),g,ncol=2,widths=c(1,3))
      cat("\n")
      cat("\n",msg1,"\n")
      cat("\n#### Results Table\n")
