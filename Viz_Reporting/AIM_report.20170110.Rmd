...
objectiveMet <- function(prop.base,relation,prop.est.lcb,prop.est.ucb) {
  ...
  low.comp <- eval(parse(text=paste(prop.est.lcb, "<=", prop.base)))
  up.comp <- eval(parse(text=paste(prop.base,"<",prop.est.ucb)))
...
for (lev in ru.levels) { # by reporting unit level
  ...
  for (ru in rus) { # by specific reporting unit
    ...
    print(xtable(prop.table),type='html',include.rownames=FALSE, html.table.attributes=list("border='0' cellpadding='5' "))
  }
}
